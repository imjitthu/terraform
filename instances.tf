...
resource "null_resource" "make_inv" {
  ...
  provisioner "local-exec" {
    command = "echo ${element(aws_instance.instance, count.index).private_ip} ${element(var.COMPONENT, count.index)} >> inv; ansible-playbook -U 'https://github.com/imjitthu/Ansible.git' -i inv Ansible/roboshop.yml"
  }
}
